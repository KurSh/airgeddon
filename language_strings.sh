...
function language_strings() {
	...
	arr["FRENCH",428]="L'attaque WEP \"Tout-en-Un\" a commencé. Pressez la touche [Entrée] dans cette console pour l'arrêter"
	arr["CATALAN",428]="L'atac WEP \"Tot-en-Un\" ha començat. Prem [Enter] a aquesta finestra per aturar-lo"
	arr["PORTUGUESE",428]="Ataque WEP \"Tudo-em-Um\" iniciado. Pressione a tecla [Enter] nesta janela para pará-lo"
	arr["RUSSIAN",428]="Атака WEP \"Все-в-Одном\" началась. Нажмите в этом окне клавишу [Enter] для её остановки"
	arr["GREEK",428]="Η επίθεση WEP \"All-in-One\" έχει αρχίσει. Πατήστε το κουμπί [Enter] σε αυτό το παράθυρο για να σταματήσει"
	...
	arr["FRENCH",429]="Clef WEP déchiffré. ASCII / Hexadécimal: ${normal_color}${wep_ascii_key}${blue_color} / ${normal_color}${wep_hex_key}${blue_color}"
	arr["CATALAN",429]="Clau WEP desencriptada amb èxit. ASCII / Hexadecimal: ${normal_color}${wep_ascii_key}${blue_color} / ${normal_color}${wep_hex_key}${blue_color}"
	...
	arr["RUSSIAN",429]="Ключ WEP расшифрован. ASCII / Шестнадцатеричный: ${normal_color}${wep_ascii_key}${blue_color} / ${normal_color}${wep_hex_key}${blue_color}"
	arr["GREEK",429]="Το κλειδί WEP αποκρυπτογραφήθηκε με επιτυχία. ASCII / Δεκαεξαδικό: ${normal_color}${wep_ascii_key}${blue_color} / ${normal_color}${wep_hex_key}${blue_color}"
	...
	arr["RUSSIAN",430]="Напечатайте путь к сохранённому файлу или нажмите [Enter] для принятия предложения по умолчанию ${normal_color}[${wep_potpath}]"
	...
	arr["GREEK",431]="Το αρχείο τρόπαιο WEP δημιουργήθηκε επιτυχώς [${normal_color}${weppotenteredpath}${blue_color}]"
	...
	arr["ENGLISH",434]="All parameters and requirements are set. The attack is going to start. Multiple windows will be opened. If you want to stop the attack manually, close the capture window or the decryption window once it appears (starting from 5000 IVs collected) and the script will automatically close them all"
	...
	arr["FRENCH",434]="Tous est prêt. L'attaque va commencer. Plusieurs consoles vont s'ouvrir. Lorsque vous voulez arrêter l'attaque, fermez manuellement la fenêtre de capture ou de déchiffrement quand elle surgit (à partir de 5000 IVs collectés) et le script arrêtera automatiquement le porcessus"
	arr["CATALAN",434]="Tots els paràmetres i requeriments estan preparats. Començarà l'atac. S'obriran múltiples finestres. Quan vulguis parar l'atac, tanca manualment la finestra de captura o la de desencriptat un cop aparegui aquesta (a partir de tenir 5000 IVs) i l'script tancarà automàticament tot"
	...
	arr["CATALAN",435]="El protocol WEP va tindre un disseny erroni i es pot trencar en pocs minuts si es genera trànsit suficient"
	...
	arr["CATALAN",436]="L'atac WEP \"Tot-en-Un\" combina diverses tècniques per generar el trànsit suficient que cal per desxifrar la clau (Caffe Latte, Hirte, Replay, Chop-Chop, Fragmentació, Falsa associació, etc.)"
	...
	arr["ENGLISH",437]="If real clients exist connected to the WEP encrypted network, the process will be significantly accelerated. Enough IVs (Initialization Vectors) will be catched faster"
	arr["SPANISH",437]="Si existen clientes reales conectados a la red con encriptación WEP, el proceso se acelerará notablemente. Se obtendrán antes suficientes IVs (Vectores de Inicialización)"
	arr["FRENCH",437]="Si des clients légitimes sont connectés à un rseau WEP le processus ira plus vite. Les Ivs requis (vecteurs d'incitiaisation) seront plus vite obtenus."
	arr["CATALAN",437]="Si hi ha clients reals connectats a la xarxa amb encriptació WEP, el procés s'accelerarà notablement. S'obtindran abans suficients IVs (Vectors d'Inicialització)"
	arr["PORTUGUESE",437]="Se clientes reais estiverem conectados à rede com criptografia WEP, o processo será acelerado significativamente. Serão obtidos IVs (Vetores de Inicialização) mais rapidamente"
	arr["RUSSIAN",437]="Если к сети с WEP шифрованием подключены реальные клиенты, то процесс будет значительно ускорен. Будет быстрее накоплено достаточное количество IVs (Векторов Инициализации)"
	arr["GREEK",437]="Εάν υπάρχουν αληθινοί συνδεδεμένοι χρήστες στο κρυπτογραφημένο WEP δίκτυο, η διαδικασία θα επιταχυνθεί σημαντικά. Θα καταγραφθούν αρκετά IVs (Initialization Vectors) γρηγορότερα"
